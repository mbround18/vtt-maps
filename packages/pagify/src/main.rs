...
async fn main() {
    ...
    let glob_path = format!("{}/**/*.info.json", &root_dir().expect("Failed to find root dir").to_str().unwrap());
    println!("{}", &glob_path);
    ...
    // println!("{}", &rendered);
    let dist_path = Path::new(&root_dir().expect("Failed to find root dir")).join("packages/gh-pagify/dist/assets");
    if !dist_path.exists() {
